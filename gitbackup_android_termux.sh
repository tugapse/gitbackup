#!/bin/bash
# script changed to work with termux apk
...
# Git pull
if $do_pull; then
  git pull origin "$branch_name" 2>&1 | tee -a $temp_location/git_log.txt # Capture output, append to log
...
if $has_diffs; then
  git add . 2>&1 | tee -a $temp_location/git_log.txt # Capture output
  ...
  git commit -m "$commit_message" 2>&1 | tee -a $temp_location/git_log.txt # Capture output, append to log

  if [ $? -eq 0 ]; then
    git push origin "$branch_name" 2>&1 | tee -a $temp_location/git_log.txt
    if [ $? -eq 0 ]; then
